...
public class MainGUI_AS {
    ...
    private void createShell() {
        ...
        shell.setText("EzLogin Authentication Server");
    ...
    private void startup() {
        PropReader propReader = new PropReader("config_as.properties");
        MainGUI_AS.props = propReader.loadProps();
        try {
            RuntimeStore.Data.listenPort = Integer.parseInt(MainGUI_AS.props.getProperty("port")); /*Listen port property*/
        ...
        RuntimeStore.Data.dbAddress = MainGUI_AS.props.getProperty("db-address");
        try {
            RuntimeStore.Data.dbPort = Integer.parseInt(MainGUI_AS.props.getProperty("db-port")); /*DB port property*/
        } catch (NumberFormatException e) {
            RuntimeStore.Data.dbPort = 5432;
            System.out.println("Invalid database port. Using default port 5432");
        }
        RuntimeStore.Data.dbUsername = MainGUI_AS.props.getProperty("db-user");
        RuntimeStore.Data.dbPassword = MainGUI_AS.props.getProperty("db-password");
        RuntimeStore.Data.serverAddress = MainGUI_AS.props.getProperty("server-address");
        try {
            RuntimeStore.Data.serverPort = Integer.parseInt(MainGUI_AS.props.getProperty("server-port"));
        } catch (NumberFormatException e) {
            RuntimeStore.Data.serverPort = 3435;
            System.out.println("Invalid authentication server port. Using default port 3435");
        }
        RuntimeStore.Data.masterToken = MainGUI_AS.props.getProperty("master-token");
